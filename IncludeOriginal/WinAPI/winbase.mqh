...
int AddIntegrityLabelToBoundaryDescriptor(HANDLE &BoundaryDescriptor,
                                          SID &IntegrityLabel);
...
int BackupRead(HANDLE file, uchar &buffer[], uint number_of_bytes_to_read,
               uint &number_of_bytes_read, int abort, int process_security,
               PVOID &context);
int BackupSeek(HANDLE file, uint low_bytes_to_seek, uint high_bytes_to_seek,
               uint &low_byte_seeked, uint &high_byte_seeked, PVOID &context);
int BackupWrite(HANDLE file, uchar &buffer[], uint number_of_bytes_to_write,
                uint &number_of_bytes_written, int abort, int process_security,
                PVOID &context);
HANDLE BeginUpdateResourceW(const string file_name,
                            int delete_existing_resources);
int BindIoCompletionCallback(HANDLE FileHandle, PVOID Function, uint Flags);
int BuildCommDCBAndTimeoutsW(const string def, DCB &lpDCB,
...
int CheckNameLegalDOS8Dot3W(const string name, char &oem_name[],
                            uint oem_name_size, int &name_contains_spaces,
                            int &name_legal);
...
int CopyFile2(const string existing_file_name, const string new_file_name,
              COPYFILE2_EXTENDED_PARAMETERS &extended_parameters);
int CopyFileExW(const string existing_file_name, const string new_file_name,
                PVOID progress_routine, PVOID data, int &cancel,
                uint copy_flags);
int CopyFileTransactedW(const string existing_file_name,
                        const string new_file_name, PVOID progress_routine,
                        PVOID data, int &cancel, uint copy_flags,
                        HANDLE transaction);
int CopyFileW(const string existing_file_name, const string new_file_name,
              int fail_if_exists);
HANDLE CreateActCtxW(const ACTCTXW &act_ctx);
int CreateDirectoryExW(const string template_directory,
                       const string new_directory, PVOID security_attributes);
int CreateDirectoryTransactedW(const string template_directory,
                               const string new_directory,
                               PVOID security_attributes, HANDLE transaction);
PVOID CreateFiber(ulong stack_size, PVOID start_address, PVOID parameter);
PVOID CreateFiberEx(ulong stack_commit_size, ulong stack_reserve_size,
                    uint flags, PVOID start_address, PVOID parameter);
HANDLE CreateFileTransactedW(const string file_name, uint desired_access,
                             uint share_mode, PVOID security_attributes,
                             uint creation_disposition,
                             uint flags_and_attributes, HANDLE template_file,
                             HANDLE transaction, ushort &mini_version,
                             PVOID extended_parameter);
int CreateHardLinkTransactedW(const string file_name,
                              const string existing_file_name,
                              PVOID security_attributes, HANDLE transaction);
int CreateHardLinkW(const string file_name, const string existing_file_name,
                    PVOID security_attributes);
int CreateJobSet(uint NumJob, JOB_SET_ARRAY &UserJobSet, uint Flags);
HANDLE CreateMailslotW(const string name, uint max_message_size,
                       uint read_timeout, PVOID security_attributes);
uchar CreateSymbolicLinkTransactedW(const string symlink_file_name,
                                    const string target_file_name, uint flags,
                                    HANDLE transaction);
uchar CreateSymbolicLinkW(const string symlink_file_name,
                          const string target_file_name, uint flags);
uint CreateTapePartition(HANDLE device, uint partition_method, uint count,
                         uint size);
...
int DequeueUmsCompletionListItems(PVOID UmsCompletionList, uint WaitTimeOut,
                                  PVOID &UmsThreadList);
uint DisableThreadProfiling(HANDLE PerformanceDataHandle);
int DnsHostnameToComputerNameW(const string hostname, ushort &computer_name[],
                               uint &size);
int DosDateTimeToFileTime(ushort fat_date, ushort fat_time,
                          FILETIME &file_time);
uint EnableThreadProfiling(HANDLE ThreadHandle, uint Flags,
                           ulong HardwareCounters,
...
int EnumResourceLanguagesW(HANDLE module, const string type, const string name,
                           PVOID enum_func, long param);
...
int FileTimeToDosDateTime(FILETIME &file_time, ushort &fat_date,
                          ushort &fat_time);
int FindActCtxSectionGuid(uint flags, const GUID &extension_guid[],
                          uint section_id, const GUID &guid_to_find[],
                          ACTCTX_SECTION_KEYED_DATA &ReturnedData);
int FindActCtxSectionStringW(uint flags, const GUID &extension_guid[],
                             uint section_id, const string string_to_find,
                             ACTCTX_SECTION_KEYED_DATA &ReturnedData);
ushort FindAtomW(const string str);
HANDLE FindFirstFileNameTransactedW(const string file_name, uint flags,
                                    uint &StringLength, string LinkName,
                                    HANDLE transaction);
HANDLE FindFirstFileTransactedW(const string file_name,
                                ...
                                HANDLE transaction);
HANDLE FindFirstStreamTransactedW(const string file_name,
                                  STREAM_INFO_LEVELS InfoLevel,
                                  FIND_STREAM_DATA &find_stream_data,
                                  uint flags, HANDLE transaction);
HANDLE FindFirstVolumeMountPointW(const string root_path_name,
                                  ushort &volume_mount_point[],
                                  uint buffer_length);
int FindNextVolumeMountPointW(HANDLE find_volume_mount_point,
                              string volume_mount_point, uint buffer_length);
int FindVolumeMountPointClose(HANDLE find_volume_mount_point);
uint FormatMessageW(uint flags, const uchar &source[], uint message_id,
                    uint language_id, ushort &buffer[], uint size,
                    PVOID &Arguments[]);
uint GetActiveProcessorCount(ushort GroupNumber);
ushort GetActiveProcessorGroupCount(void);
int GetApplicationRecoveryCallback(HANDLE process, PVOID &recovery_callback,
                                   PVOID &parameter, uint &ping_interval,
                                   uint &flags);
int GetApplicationRestartSettings(HANDLE process, ushort &commandline[],
...
uint GetCompressedFileSizeTransactedW(const string file_name,
                                      uint &file_size_high, HANDLE transaction);
...
int GetFileAttributesTransactedW(const string file_name,
                                 GET_FILEEX_INFO_LEVELS info_level_id,
                                 PVOID file_information, HANDLE transaction);
int GetFileBandwidthReservation(HANDLE file, uint &period_milliseconds,
                                uint &bytes_per_period, int &discardable,
                                uint &transfer_size,
                                uint &num_outstanding_requests);
int GetFileInformationByHandleEx(HANDLE file,
                                 FILE_INFO_BY_HANDLE_CLASS FileInformationClass,
                                 PVOID file_information, uint buffer_size);
int GetFileInformationByHandleEx(HANDLE file,
                                 FILE_INFO_BY_HANDLE_CLASS FileInformationClass,
                                 uchar &file_information[], uint buffer_size);
uint GetFirmwareEnvironmentVariableExW(const string name, const string guid,
                                       PVOID buffer, uint size,
                                       uint &attribubutes);
uint GetFirmwareEnvironmentVariableW(const string name, const string guid,
                                     PVOID buffer, uint size);
int GetFirmwareType(FIRMWARE_TYPE &FirmwareType);
uint GetFullPathNameTransactedW(const string file_name, uint buffer_length,
                                string buffer, string &file_part,
                                HANDLE transaction);
uint GetLongPathNameTransactedW(const string short_path, string long_path,
                                uint buffer, HANDLE transaction);
int GetMailslotInfo(HANDLE mailslot, uint &max_message_size, uint &next_size,
                    uint &message_count, uint &read_timeout);
...
uint GetPrivateProfileIntW(const string app_name, const string key_name,
                           int default_value, const string file_name);
uint GetPrivateProfileSectionNamesW(string return_buffer, uint size,
                                    const string file_name);
uint GetPrivateProfileSectionW(const string app_name, string returned_string,
                               uint size, const string file_name);
uint GetPrivateProfileStringW(const string app_name, const string key_name,
                              const string default_value,
                              string returned_string, uint size,
                              const string file_name);
int GetPrivateProfileStructW(const string section, const string key,
                             PVOID struct_obj, uint size_struct,
                             const string file);
int GetProcessAffinityMask(HANDLE process, ulong &process_affinity_mask,
...
int GetProcessWorkingSetSize(HANDLE process, ulong &minimum_working_set_size,
                             ulong &maximum_working_set_size);
uint GetProfileIntW(const string app_name, const string key_name,
                    int default_value);
uint GetProfileSectionW(const string app_name, string returned_string,
                        uint size);
uint GetProfileStringW(const string app_name, const string key_name,
                       const string default_value, string returned_string,
                       uint size);
...
uint GetTapeParameters(HANDLE device, uint operation, uint &size,
                       PVOID tape_information);
uint GetTapePosition(HANDLE device, uint position_type, uint &partition,
                     uint &offset_low, uint &offset_high);
uint GetTapeStatus(HANDLE device);
int GetThreadSelectorEntry(HANDLE thread, uint selector,
                           LDT_ENTRY &selector_entry);
int GetUmsCompletionListEvent(PVOID UmsCompletionList,
                              HANDLE &UmsCompletionEvent);
int GetUmsSystemThreadInformation(
    HANDLE ThreadHandle, UMS_SYSTEM_THREAD_INFORMATION &SystemThreadInfo);
...
int InitializeContext(uchar &Buffer[], uint ContextFlags, CONTEXT &Context,
                      uint &ContextLength);
int InitializeContext(PVOID Buffer, uint ContextFlags, CONTEXT &Context,
                      uint &ContextLength);
...
int MapUserPhysicalPagesScatter(PVOID &VirtualAddresses[], ulong NumberOfPages,
                                ulong &PageArray[]);
PVOID MapViewOfFileExNuma(HANDLE file_mapping_object, uint desired_access,
                          uint file_offset_high, uint file_offset_low,
                          ulong number_of_bytes_to_map, PVOID base_address,
                          uint preferred);
int MoveFileExW(const string existing_file_name, const string new_file_name,
                uint flags);
int MoveFileTransactedW(const string existing_file_name,
                        const string new_file_name, PVOID progress_routine,
                        PVOID data, uint flags, HANDLE transaction);
int MoveFileW(const string existing_file_name, const string new_file_name);
int MoveFileWithProgressW(const string existing_file_name,
                          const string new_file_name, PVOID progress_routine,
                          PVOID data, uint flags);
int MulDiv(int number, int numerator, int denominator);
HANDLE OpenFileById(HANDLE volume_hint, FILE_ID_DESCRIPTOR &file_id,
                    uint desired_access, uint share_mode,
                    PVOID security_attributes, uint flags_and_attributes);
...
int QueryUmsThreadInformation(PVOID UmsThread,
                              RTL_UMS_THREAD_INFO_CLASS UmsThreadInfoClass,
                              PVOID UmsThreadInformation,
                              uint UmsThreadInformationLength,
                              uint &ReturnLength);
int ReadDirectoryChangesExW(HANDLE directory, PVOID buffer, uint buffer_length,
                            int watch_subtree, uint notify_filter,
                            uint &bytes_returned, OVERLAPPED &overlapped,
                            PVOID completion_routine,
                            READ_DIRECTORY_NOTIFY_INFORMATION_CLASS
                                ReadDirectoryNotifyInformationClass);
int ReadDirectoryChangesW(HANDLE directory, PVOID buffer, uint buffer_length,
                          int watch_subtree, uint notify_filter,
                          uint &bytes_returned, OVERLAPPED &overlapped,
                          PVOID completion_routine);
uint ReadThreadProfilingData(HANDLE PerformanceDataHandle, uint Flags,
                             PERFORMANCE_DATA &PerformanceData);
int RegisterApplicationRecoveryCallback(PVOID recovey_callback, PVOID parameter,
                                        uint ping_interval, uint flags);
int RegisterApplicationRestart(const string commandline, uint flags);
int RegisterWaitForSingleObject(HANDLE &new_wait_object, HANDLE object,
                                PVOID Callback, PVOID Context,
                                uint milliseconds, uint flags);
...
HANDLE ReOpenFile(HANDLE original_file, uint desired_access, uint share_mode,
                  uint flags_and_attributes);
int ReplaceFileW(const string replaced_file_name,
                 const string replacement_file_name,
                 const string backup_file_name, uint replace_flags,
                 PVOID exclude, PVOID reserved);
int ReplacePartitionUnit(string TargetPartition, string SparePartition,
                         uint Flags);
...
int SetFileAttributesTransactedW(const string file_name, uint file_attributes,
                                 HANDLE transaction);
int SetFileBandwidthReservation(HANDLE file, uint period_milliseconds,
                                uint bytes_per_period, int discardable,
                                uint &transfer_size,
                                uint &num_outstanding_requests);
...
int SetFirmwareEnvironmentVariableExW(const string name, const string guid,
                                      PVOID value, uint size, uint attributes);
int SetFirmwareEnvironmentVariableW(const string name, const string guid,
                                    PVOID value, uint size);
...
int SetProcessWorkingSetSize(HANDLE process, ulong minimum_working_set_size,
                             ulong maximum_working_set_size);
...
uint SetTapePosition(HANDLE device, uint position_method, uint partition,
                     uint offset_low, uint offset_high, int immediate);
PVOID SetThreadAffinityMask(HANDLE thread, PVOID thread_affinity_mask);
uint SetThreadExecutionState(uint flags);
HANDLE SetTimerQueueTimer(HANDLE TimerQueue, PVOID Callback, PVOID Parameter,
                          uint DueTime, uint Period, int PreferIo);
int SetUmsThreadInformation(PVOID UmsThread,
                            RTL_UMS_THREAD_INFO_CLASS UmsThreadInfoClass,
                            PVOID UmsThreadInformation,
                            uint UmsThreadInformationLength);
...
int SetVolumeMountPointW(const string volume_mount_point,
                         const string volume_name);
int SetXStateFeaturesMask(CONTEXT &Context, ulong FeatureMask);
uint SignalObjectAndWait(HANDLE object_to_signal, HANDLE object_to_wait_on,
...
int UpdateResourceW(HANDLE update, const string type, const string name,
                    ushort &language, PVOID data, uint cb);
int VerifyVersionInfoW(OSVERSIONINFOEXW &version_information, uint type_mask,
...
int Wow64GetThreadSelectorEntry(HANDLE thread, uint selector,
                                LDT_ENTRY &selector_entry);
int Wow64SetThreadContext(HANDLE thread, CONTEXT &context);
uint Wow64SuspendThread(HANDLE thread);
int WritePrivateProfileSectionW(const string app_name, const string str,
                                const string file_name);
int WritePrivateProfileStringW(const string app_name, const string key_name,
                               const string str, const string file_name);
int WritePrivateProfileStructW(const string section, const string key,
                               PVOID struct_obj, uint size_struct,
                               const string file);
int WriteProfileSectionW(const string app_name, const string str);
int WriteProfileStringW(const string app_name, const string key_name,
                        const string str);
uint WriteTapemark(HANDLE device, uint tapemark_type, uint tapemark_count,
                   int immediate);
...
int GetEventLogInformation(HANDLE event_log, uint info_level, PVOID buffer,
...
int LogonUserExW(const string username, const string domain,
                 const string password, uint logon_type, uint logon_provider,
                 HANDLE &token, PVOID &logon_sid, PVOID &profile_buffer,
                 uint &profile_length, QUOTA_LIMITS &quota_limits);
int LogonUserW(const string username, const string domain,
               const string password, uint logon_type, uint logon_provider,
               HANDLE &token);
int LookupAccountNameW(const string system_name, const string account_name,
                       SID &Sid, uint &sid, string ReferencedDomainName,
                       uint &referenced_domain_name, SID_NAME_USE &use);
int LookupAccountSidW(const string system_name, SID &Sid, string Name,
...
HANDLE OpenBackupEventLogW(const string lpUNCServerName,
                           const string file_name);
...
uint ReadEncryptedFileRaw(PVOID export_callback, PVOID callback_context,
                          PVOID context);
int ReadEventLogW(HANDLE event_log, uint read_flags, uint record_offset,
                  PVOID buffer, uint number_of_bytes_to_read, uint &bytes_read,
                  uint &min_number_of_bytes_needed);
HANDLE RegisterEventSourceW(const string lpUNCServerName,
                            const string source_name);
int ReportEventW(HANDLE event_log, ushort &type, ushort &category,
                 uint dwEventID, SID &user_sid, ushort &num_strings,

...
HANDLE CreateThread(PVOID thread_attributes, ulong stack_size,
                    PVOID start_address, PVOID parameter, uint creation_flags,
                    uint &thread_id);
HANDLE CreateRemoteThread(HANDLE process, PVOID thread_attributes,
                          ulong stack_size, PVOID start_address,
                          PVOID parameter, uint creation_flags,
                          uint &thread_id);
...
int CreateProcessW(const string application_name, string command_line,
                   PVOID process_attributes, PVOID thread_attributes,
                   int inherit_handles, uint creation_flags, PVOID environment,
                   const string current_directory, STARTUPINFOW &startup_info,
...
int InitializeProcThreadAttributeList(PVOID attribute_list,
                                      uint attribute_count, uint flags,
                                      ulong &size);
...
int UpdateProcThreadAttribute(PVOID attribute_list, uint flags, uint attribute,
                              PVOID value, ulong size, PVOID previous_value,
                              ulong &return_size);
HANDLE CreateRemoteThreadEx(HANDLE process, PVOID thread_attributes,
                            ulong stack_size, PVOID start_address,
                            PVOID parameter, uint creation_flags,
                            PVOID attribute_list, uint &thread_id);
void GetCurrentThreadStackLimits(ulong &low_limit, ulong &high_limit);
int GetThreadContext(HANDLE thread, CONTEXT &context);
int GetProcessMitigationPolicy(HANDLE process,
                               PROCESS_MITIGATION_POLICY mitigation_policy,
                               PVOID buffer, ulong length);
int SetThreadContext(HANDLE thread, const CONTEXT &context);
int SetProcessMitigationPolicy(PROCESS_MITIGATION_POLICY mitigation_policy,
                               PVOID buffer, ulong length);
int FlushInstructionCache(HANDLE process, const PVOID base_address, ulong size);
int GetThreadTimes(HANDLE thread, FILETIME &creation_time, FILETIME &exit_time,
...
int SetThreadIdealProcessorEx(HANDLE thread, PROCESSOR_NUMBER &ideal_processor,
...
int GetSystemTimes(FILETIME &idle_time, FILETIME &kernel_time,
                   FILETIME &user_time);
int GetThreadInformation(HANDLE thread,
                         THREAD_INFORMATION_CLASS thread_information_class,
                         PVOID thread_information,
                         uint thread_information_size);
int SetThreadInformation(HANDLE thread,
                         THREAD_INFORMATION_CLASS thread_information_class,
                         PVOID thread_information,
                         uint thread_information_size);
int IsProcessCritical(HANDLE process, int &critical);
int SetProtectedPolicy(const GUID &policy_guid, ulong policy_value,
                       ulong &old_policy_value);
...
int SetProcessInformation(HANDLE process,
                          PROCESS_INFORMATION_CLASS process_information_class,
                          PVOID process_information,
                          uint process_information_size);
int GetProcessInformation(HANDLE process,
                          PROCESS_INFORMATION_CLASS process_information_class,
                          PVOID process_information,
                          uint process_information_size);
int GetSystemCpuSetInformation(SYSTEM_CPU_SET_INFORMATION &information,
                               uint buffer_length, ulong returned_length,
                               HANDLE process, uint flags);
int GetProcessDefaultCpuSets(HANDLE process, ulong &cpu_set_ids,
                             uint cpu_set_id_count, ulong required_id_count);
int SetProcessDefaultCpuSets(HANDLE process, const uint &cpu_set_ids,
                             uint cpu_set_id_count);
int GetThreadSelectedCpuSets(HANDLE thread, ulong &cpu_set_ids,
                             uint cpu_set_id_count, ulong required_id_count);
int SetThreadSelectedCpuSets(HANDLE thread, const uint &cpu_set_ids,
                             uint cpu_set_id_count);
...
int CreateProcessAsUserW(HANDLE token, const string application_name,
                         ...
                         uint creation_flags, PVOID environment,
                         const string current_directory,
                         STARTUPINFOW &startup_info,
                         PROCESS_INFORMATION &process_information);
int SetThreadToken(HANDLE thread, HANDLE token);
int OpenProcessToken(HANDLE process_handle, uint desired_access,
                     HANDLE &token_handle);

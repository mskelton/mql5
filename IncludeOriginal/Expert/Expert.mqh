...
enum ENUM_TRADE_EVENTS {
  ...
  TRADE_EVENT_POSITION_OPEN =
      0x1, // flag of expecting the "opening of position" event
  TRADE_EVENT_POSITION_VOLUME_CHANGE =
      0x2, // flag of expecting of the "modification of position volume" event
  TRADE_EVENT_POSITION_MODIFY = 0x4, // flag of expecting of the "modification
                                     // of stop order of position" event
  TRADE_EVENT_POSITION_CLOSE =
      0x8, // flag of expecting of the "closing of position" event
  TRADE_EVENT_POSITION_STOP_TAKE =
      0x10, // flag of expecting of the "triggering of stop order of position"
  TRADE_EVENT_ORDER_PLACE =
      0x20, // flag of expecting of the "placing of pending order" event
  TRADE_EVENT_ORDER_MODIFY =
      0x40, // flag of expecting of the "modification of pending order" event
  TRADE_EVENT_ORDER_DELETE =
      0x80, // flag of expecting of the "deletion of pending order" event
  TRADE_EVENT_ORDER_TRIGGER =
      0x100 // flag of expecting of the "triggering of pending order" event
};
//+------------------------------------------------------------------+
//| Macro definitions.                                               |
//+------------------------------------------------------------------+
...
protected:
  ...
  bool m_on_chart_event_process; // OnChartEvent will be processed (default
                                 // false)
  bool
      m_on_book_event_process; // OnBookEvent will be processed  (default false)
...
bool CExpert::Init(string symbol, ENUM_TIMEFRAMES period, bool every_tick,
                   ulong magic) {
  //--- returns false if the EA is initialized on a symbol/timeframe different
...
bool CExpert::InitTrade(ulong magic, CExpertTrade *trade = NULL) {
  //--- ÑÐ´Ð°Ð»ÑÐµÐ¼ ÑÑÑÐµÑÑÐ²ÑÑÑÐ¸Ð¹
  //Ð¾Ð±ÑÐµÐºÑ
  if (m_trade != NULL)
    delete m_trade;
  //---
...
bool CExpert::CheckClose(void) {
  ...
  if (m_position.PositionType() == POSITION_TYPE_BUY) {
    //--- check the possibility of closing the long position / delete pending
    //orders to buy
    if (CheckCloseLong()) {
      DeleteOrdersLong();
      return (true);
    ...
    //--- check the possibility of closing the short position / delete pending
    //orders to sell
    if (CheckCloseShort()) {
      DeleteOrdersShort();
      return (true);

...
float DXFresnelTerm(float costheta, float refractionindex) {
  float g = (float)sqrt(refractionindex * refractionindex +
                        costheta * costheta - 1.0f);
  float a = g + costheta;
  float d = g - costheta;
  float result = (costheta * a - 1.0f) * (costheta * a - 1.0f) /
                     ((costheta * d + 1.0f) * (costheta * d + 1.0f)) +
                 1.0f;
  result *= 0.5f * d * d / (a * a);
...
void DXVec2CatmullRom(DXVector2 &pout, const DXVector2 &pv0,
  ...
  pout.x =
      0.5f * (2.0f * pv1.x + (pv2.x - pv0.x) * s +
              (2.0f * pv0.x - 5.0f * pv1.x + 4.0f * pv2.x - pv3.x) * s * s +
              (pv3.x - 3.0f * pv2.x + 3.0f * pv1.x - pv0.x) * s * s * s);
  pout.y =
      0.5f * (2.0f * pv1.y + (pv2.y - pv0.y) * s +
              (2.0f * pv0.y - 5.0f * pv1.y + 4.0f * pv2.y - pv3.y) * s * s +
...
void DXVec3CatmullRom(DXVector3 &pout, const DXVector3 &pv0,
  ...
  pout.y =
      0.5f * (2.0f * pv1.y + (pv2.y - pv0.y) * s +
              ...
              (pv3.y - 3.0f * pv2.y + 3.0f * pv1.y - pv0.y) * s * s * s);
  pout.z =
      0.5f * (2.0f * pv1.z + (pv2.z - pv0.z) * s +
              (2.0f * pv0.z - 5.0f * pv1.z + 4.0f * pv2.z - pv3.z) * s * s +
...
void DXVec3Transform(DXVector4 &pout, const DXVector3 &pv, const DXMatrix &pm) {
  ...
  out.x =
      pm.m[0][0] * pv.x + pm.m[1][0] * pv.y + pm.m[2][0] * pv.z + pm.m[3][0];
  out.y =
      pm.m[0][1] * pv.x + pm.m[1][1] * pv.y + pm.m[2][1] * pv.z + pm.m[3][1];
  out.z =
      pm.m[0][2] * pv.x + pm.m[1][2] * pv.y + pm.m[2][2] * pv.z + pm.m[3][2];
  out.w =
      pm.m[0][3] * pv.x + pm.m[1][3] * pv.y + pm.m[2][3] * pv.z + pm.m[3][3];
  pout = out;
}
...
void DXVec4CatmullRom(DXVector4 &pout, const DXVector4 &pv0,
  ...
  pout.z =
      0.5f * (2.0f * pv1.z + (pv2.z - pv0.z) * s +
              ...
              (pv3.z - 3.0f * pv2.z + 3.0f * pv1.z - pv0.z) * s * s * s);
  pout.w =
      0.5f * (2.0f * pv1.w + (pv2.w - pv0.w) * s +
              (2.0f * pv0.w - 5.0f * pv1.w + 4.0f * pv2.w - pv3.w) * s * s +
...
void DXVec4Transform(DXVector4 &pout, const DXVector4 &pv, const DXMatrix &pm) {
  DXVector4 temp;
  temp.x = pm.m[0][0] * pv.x + pm.m[1][0] * pv.y + pm.m[2][0] * pv.z +
           pm.m[3][0] * pv.w;
  temp.y = pm.m[0][1] * pv.x + pm.m[1][1] * pv.y + pm.m[2][1] * pv.z +
           pm.m[3][1] * pv.w;
  temp.z = pm.m[0][2] * pv.x + pm.m[1][2] * pv.y + pm.m[2][2] * pv.z +
           pm.m[3][2] * pv.w;
  temp.w = pm.m[0][3] * pv.x + pm.m[1][3] * pv.y + pm.m[2][3] * pv.z +
           pm.m[3][3] * pv.w;
  pout = temp;
}
...
float DXMatrixDeterminant(const DXMatrix &pm) {
  ...
  return (pm.m[0][0] * v[0] + pm.m[0][1] * v[1] + pm.m[0][2] * v[2] +
          pm.m[0][3] * v[3]);
}
...
void DXMatrixInverse(DXMatrix &pout, float &pdeterminant, const DXMatrix &pm) {
  ...
  float det = pm.m[0][0] * v[0] + pm.m[0][1] * v[4] + pm.m[0][2] * v[8] +
              pm.m[0][3] * v[12];
  if (det == 0.0f) {
    for (int j = 0; j < 4; j++)
      for (int i = 0; i < 4; i++) {
...
void DXMatrixMultiply(DXMatrix &pout, const DXMatrix &pm1,
                      const DXMatrix &pm2) {
  DXMatrix out = {};
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
...
void DXMatrixMultiplyTranspose(DXMatrix &pout, const DXMatrix &pm1,
  ...
  for (int i = 0; i < 4; i++)
    for (int j = 0; j < 4; j++)
      temp.m[j][i] = pm1.m[i][0] * pm2.m[0][j] + pm1.m[i][1] * pm2.m[1][j] +
                     pm1.m[i][2] * pm2.m[2][j] + pm1.m[i][3] * pm2.m[3][j];
  pout = temp;
}
...
void DXMatrixTransformation2D(DXMatrix &pout, const DXVector2 &pscalingcenter,
  ...
  DXMatrixTransformation(pout, sca_center, sca_rot, sca, rot_center, rot,
                         trans);
}
...
void DXPlaneTransform(DXPlane &pout, const DXPlane &pplane,
                      const DXMatrix &pm) {
  DXPlane plane = pplane;

  pout.a = pm.m[0][0] * plane.a + pm.m[1][0] * plane.b + pm.m[2][0] * plane.c +
           pm.m[3][0] * plane.d;
  pout.b = pm.m[0][1] * plane.a + pm.m[1][1] * plane.b + pm.m[2][1] * plane.c +
           pm.m[3][1] * plane.d;
  pout.c = pm.m[0][2] * plane.a + pm.m[1][2] * plane.b + pm.m[2][2] * plane.c +
           pm.m[3][2] * plane.d;
  pout.d = pm.m[0][3] * plane.a + pm.m[1][3] * plane.b + pm.m[2][3] * plane.c +
           pm.m[3][3] * plane.d;
}
...
void weightedcapintegrale(float &out[], uint order, float angle) {
  ...
  out[4] =
      -0.25f * DX_PI * coeff[0] * (7.0f * coeff[2] - 10.0f * coeff[1] + 3.0f);
  if (order == 5)
    return;

  out[5] = DX_PI * (-2.625f * coeff[2] * coeff[1] + 4.375f * coeff[2] -
                    1.875f * coeff[1] + 0.125f);
}
...
int DXSHEvalConeLight(int order, const DXVector3 &dir, float radius,
  ...
  if (radius <= 0.0f)
    return (DXSHEvalDirectionalLight(order, dir, Rintensity, Gintensity,
                                     Bintensity, rout, gout, bout));
...
void DXSHEvalDirection(float &out[], int order, const DXVector3 &dir) {
  ...
  out[20] = 3.0f / (16.0f * (float)sqrt(DX_PI)) *
            (35.0f * dirzzzz - 30.f * dirzz + 3.0f);
  out[21] = 0.375f * (float)sqrt(10.0f / DX_PI) * dirxz * (3.0f - 7.0f * dirzz);
  out[22] = 0.375f * (float)sqrt(5.0f / DX_PI) * (dirxx - diryy) *
            (7.0f * dirzz - 1.0f);
  out[23] = 3.0f * dir.z * out[15];
  out[24] = 3.0f / 16.0f * float(sqrt(35.0f / DX_PI)) *
            (dirxxxx - 6.0f * dirxyxy + diryyyy);
  if (order == 5)
    return;

  out[25] = -3.0f / 32.0f * (float)sqrt(154.0f / DX_PI) * dir.y *
            (5.0f * dirxxxx - 10.0f * dirxyxy + diryyyy);
  out[26] =
      0.75f * (float)sqrt(385.0f / DX_PI) * dirxy * dir.z * (dirxx - diryy);
  out[27] = (float)sqrt(770.0f / DX_PI) / 32.0f * dir.y *
            (3.0f * dirxx - diryy) * (1.0f - 9.0f * dirzz);
  out[28] = (float)sqrt(1155.0f / DX_PI) / 4.0f * dirxy * dir.z *
            (3.0f * dirzz - 1.0f);
  out[29] = (float)sqrt(165.0f / DX_PI) / 16.0f * dir.y *
            (14.0f * dirzz - 21.0f * dirzzzz - 1.0f);
  out[30] = (float)sqrt(11.0f / DX_PI) / 16.0f * dir.z *
            (63.0f * dirzzzz - 70.0f * dirzz + 15.0f);
  out[31] = (float)sqrt(165.0f / DX_PI) / 16.0f * dir.x *
            (14.0f * dirzz - 21.0f * dirzzzz - 1.0f);
  out[32] = (float)sqrt(1155.0f / DX_PI) / 8.0f * dir.z * (dirxx - diryy) *
            (3.0f * dirzz - 1.0f);
  out[33] = (float)sqrt(770.0f / DX_PI) / 32.0f * dir.x *
            (dirxx - 3.0f * diryy) * (1.0f - 9.0f * dirzz);
  out[34] = 3.0f / 16.0f * (float)sqrt(385.0f / DX_PI) * dir.z *
            (dirxxxx - 6.0f * dirxyxy + diryyyy);
  out[35] = -3.0f / 32.0f * (float)sqrt(154.0f / DX_PI) * dir.x *
            (dirxxxx - 10.0f * dirxyxy + 5.0f * diryyyy);
}
...
int DXSHEvalSphericalLight(int order, const DXVector3 &dir, float radius,
  ...
  float clamped_angle =
      (dist <= radius) ? DX_PI / 2.0f : (float)asin(radius / dist);
...
void rotate_X(float &out[], uint order, float a, float &in[]) {
  ...
  out[25] = a * 0.7015607357f * in[30] - a * 0.6846531630f * in[32] +
            a * 0.1976423711f * in[34];
  out[26] = -0.5f * in[26] + 0.8660253882f * in[28];
  out[27] = a * 0.5229125023f * in[30] + a * 0.3061861992f * in[32] -
            a * 0.7954951525f * in[34];
  out[28] = 0.8660253882f * in[26] + 0.5f * in[28];
  out[29] = a * 0.4841229022f * in[30] + a * 0.6614378691f * in[32] +
            a * 0.5728219748f * in[34];
  out[30] = -a * 0.7015607357f * in[25] - a * 0.5229125023f * in[27] -
            a * 0.4841229022f * in[29];
  out[31] = 0.125f * in[31] + 0.4050463140f * in[33] + 0.9057110548f * in[35];
  out[32] = a * 0.6846531630f * in[25] - a * 0.3061861992f * in[27] -
            a * 0.6614378691f * in[29];
  out[33] = 0.4050463140f * in[31] + 0.8125f * in[33] - 0.4192627370f * in[35];
  out[34] = -a * 0.1976423711f * in[25] + a * 0.7954951525f * in[27] -
            a * 0.5728219748f * in[29];
  out[35] =
      0.9057110548f * in[31] - 0.4192627370f * in[33] + 0.0624999329f * in[35];
}

void DXSHRotate(float &out[], int order, const DXMatrix &mat,
                const float &in[]) {
  ...
  if (order <= 3) {
    ...
    if (order == 3) {
      ...
      out[6] =
          (mat.m[2][2] * mat.m[2][2] - 0.5f * (coeff[4] + coeff[5])) * in[6];
      out[6] -= (0.5773502692f * (coeff[0] + coeff[1]) -
                 1.1547005384f * mat.m[1][2] * mat.m[0][2]) *
                in[4];
      out[6] += (0.5773502692f * (coeff[2] + coeff[3]) -
                 1.1547005384f * mat.m[1][2] * mat.m[2][2]) *
                in[5];
      out[6] += (0.5773502692f * (coeff[6] + coeff[7]) -
                 1.1547005384f * mat.m[0][2] * mat.m[2][2]) *
                in[7];
      out[6] += (0.2886751347f * (coeff[9] - coeff[8] + coeff[10] - coeff[11]) -
                 0.5773502692f *
                     (mat.m[1][2] * mat.m[1][2] - mat.m[0][2] * mat.m[0][2])) *
                in[8];

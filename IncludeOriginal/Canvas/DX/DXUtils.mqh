...
bool DXLoadObjData(const string filename, DXVector4 &v_positions[],
  ...
  while (!FileIsEnding(file_handle)) {
    ...
    else if (str_parsed[0] == "VT")
      data_type = OBJ_DATA_VT;
    else if (str_parsed[0] == "VN")
      data_type = OBJ_DATA_VN;
    else if (str_parsed[0] == "F")
    ...
    case OBJ_DATA_V: {
      ...
      if (show_debug)
        printf("posiiton %d:  %f   %f   %f", idx, v_positions[idx].x,
               v_positions[idx].y, v_positions[idx].z);
    ...
    case OBJ_DATA_VN: {
      ...
      if (show_debug)
        printf("normal %d:  %f   %f   %f", idx, v_normals[idx].x,
               v_normals[idx].y, v_normals[idx].z);
    ...
    case OBJ_DATA_F: {
      ...
      if (show_debug)
        printf("%face d: %d vertex(%d,%d,%d,%d)  texture(%d,%d,%d,%d)  "
               "normal(%d,%d,%d,%d)",
               idx + 1, faces[idx].v[0], faces[idx].v[1], faces[idx].v[2],
               faces[idx].v[3], faces[idx].t[0], faces[idx].t[1],
               faces[idx].t[2], faces[idx].t[3], faces[idx].n[0],
               faces[idx].n[1], faces[idx].n[2], faces[idx].n[3]);
  ...
  if (show_debug) {
    printf("File %s loaded successfully. Total lines: %d", filename,
           total_lines);
...
bool DXLoadObjModel(const string filename, TVertex &vertices[], uint &indices[],
  ...
  if (!DXLoadObjData(filename, v_positions, v_tcoords, v_normals, faces,
                     false)) {
  ...
  for (int i = 0; i < total_faces; i++) {
    ...
    for (int j = 0; j < faces[i].total; j++) {
      if (faces[i].v[j] <= 0 || faces[i].v[j] > total_v_positions) {
        printf("Error in %d face, %d vertext index is %d. Total posiitons=%d",
               i, j, faces[i].v[j], total_v_positions);
        return (false);
      }
      if (total_v_tcoords > 0) {
        if (faces[i].t[j] <= 0 || faces[i].t[j] > total_v_tcoords) {
          printf("Error in %d face, %d tcoord index is %d. Total tcoords=%d", i,
                 j, faces[i].v[j], total_v_positions);
          return (false);
        }
      ...
      if (total_v_normals > 0) {
        if (faces[i].n[j] <= 0 || faces[i].n[j] > total_v_normals) {
          printf("Error in %d face, %d normal index is %d. Total normals=%d", i,
                 j, faces[i].v[j], total_v_positions);
          return (false);
        }
...
bool DXComputeBox(const DXVector3 &from, const DXVector3 &to,
  ...
  uint ind[] = {0,  1,  2,  2,  3,  0,  4,  5,  6,  6,  7,  4,
  ...
  float left = from.x, right = to.x, bottom = from.y, top = to.y, near = from.z,
        far = to.z;
  if (from.x > to.x) {
    right = from.x;
    left = to.x;
...
bool DXComputeSphere(float radius, uint tessellation, TVertex &vertices[],
  ...
  for (uint i = 0, idx = 0; i <= segments_y; i++) {
    ...
    for (uint j = 0; j <= segments_xz; j++, idx++) {
      ...
      DXVector3 normal = DXVector3((float)sin(longitude) * dxz, dy,
                                   (float)cos(longitude) * dxz);
...
bool DXComputeTorus(float outer_radius, float inner_radius, uint tessellation,
  ...
  for (uint i = 0; i <= tessellation; i++) {
    ...
    DXMatrixRotationY(rotation,
                      -1.0f * (i % tessellation) * DX_PI_MUL2 / tessellation -
                          DX_PI_DIV2);
    ...
    for (uint j = 0; j <= tessellation; j++) {
      ...
      vertices[v].normal =
          DXVector4((float)cos(angle), (float)sin(angle), 0.0f, 0.0f);
      vertices[v].position =
          DXVector4(vertices[v].normal.x * inner_radius,
                    vertices[v].normal.y * inner_radius, 0.0f, 1.0f);
...
bool DXComputeCylinder(float radius, float height, uint tessellation,
                       TVertex &vertices[], uint &indices[]) {
  return (DXComputeTruncatedCone(radius, radius, height, tessellation, vertices,
                                 indices));
}
...
bool DXComputeTruncatedCone(float radius_top, float radius_bottom, float height,
  ...
  for (uint i = 0; i <= tessellation; i++) {
    ...
    vertices[v].normal =
        DXVector4(dx * normal.x, normal.y, dz * normal.x, 0.0f);
    vertices[v].position =
        DXVector4(dx * radius_bottom, -dy, dz * radius_bottom, 1.0f);
    vertices[v].tcoord = DXVector2(u, 1.0f);
    v++;
    vertices[v].normal = vertices[v - 1].normal;
    vertices[v].position =
        DXVector4(dx * radius_top, dy, dz * radius_top, 1.0f);
...
bool DXComputeSurface(double &data[], uint data_width, uint data_height,
  ...
  for (uint j = 0; j < data_height; j++) {
    for (uint i = 0; i < data_width; i++) {
      ...
      vertices[idx].tcoord =
          DXVector2(i * step_x / texture_size.x, j * step_z / texture_size.y);
    }
  }
  ...
  if (use_normals) {
    for (uint j = 0; j < data_height; j++) {
      for (uint i = 0; i < data_width; i++) {
        ...
        if (i <= 0)
          DXVec4Subtract(v1, vertices[j * data_width + i].position,
                         vertices[j * data_width + i + 1].position);
        else if (i >= data_width - 1)
          DXVec4Subtract(v1, vertices[j * data_width + i - 1].position,
                         vertices[j * data_width + i].position);
        else
          DXVec4Subtract(v1, vertices[j * data_width + i - 1].position,
                         vertices[j * data_width + i + 1].position);

        if (j <= 0)
          DXVec4Subtract(v2, vertices[j * data_width + i].position,
                         vertices[(j + 1) * data_width + i].position);
        else if (j >= data_height - 1)
          DXVec4Subtract(v2, vertices[(j - 1) * data_width + i].position,
                         vertices[j * data_width + i].position);
        else
          DXVec4Subtract(v2, vertices[(j - 1) * data_width + i].position,
                         vertices[(j + 1) * data_width + i].position);
        ...
        float inv_len =
            (float)(1.0 / sqrt(normal.x * normal.x + normal.y * normal.y +
                               normal.z * normal.z));
...
void DXComputeColorRedToGreen(const float value, DXColor &cout) {
  if (value <= 0.5) {
    ...
    cout.g =
        DXScalarLerp(0.6f, 0.95f, fmin(fmax(2.0f - 2 * value, 0.0f), 1.0f));
  }
  cout.b = 0.0f;
  cout.a = 1.0f;
}

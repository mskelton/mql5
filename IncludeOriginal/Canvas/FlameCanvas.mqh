...
CFlameCanvas::CFlameCanvas(void)
    : m_bar_gap(16), m_bar_width(8), m_chart_scale(1), m_chart_price_min(0.0),
...
bool CFlameCanvas::FlameCreate(const string name, const datetime time,
                               const int future_bars, const int back_bars) {

  ChartScale();

  int width = (int)m_bar_gap * (future_bars + back_bars);
  int height = (int)ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS);
  if (!CreateBitmap(0, 0, name, time - back_bars * PeriodSeconds(),
                    m_chart_price_max, width, height,
  ...
  if (m_rates_total == 0)
    ...
    if (m_rates_total != value) {
      ...
      ObjectSetInteger(0, m_objname, OBJPROP_TIME, 0,
                       ObjectGetInteger(0, m_objname, OBJPROP_TIME) +
                           (value - m_rates_total) * PeriodSeconds());
      m_rates_total = value;
    }
  }
}
...
void CFlameCanvas::PaletteSet(uint clr) {
  ...
  for (uint a, i = 0; i < 256; i++) {
    ...
    m_palette[i] =
        (a << 24) | (uint(255) << 16) | (uint(g + 0.5) << 8) | uint(b + 0.5);
...
void CFlameCanvas::FlameDraw(const double &prices[], const int width,
                             const int lenght) {
  static GRADIENT_SIZE sword[] = {{100, 0}, {150, 70}, {0, 100}};
  static GRADIENT_COLOR flame[] = {
      {0x00, 0},      {0x7F7F7F, 12}, {0xCCCCCC, 30}, {0xFFFFFF, 45},
      {0xFFFFFF, 55}, {0xCCCCCC, 70}, {0x7F7F7F, 88}, {0x00, 100}};
...
void CFlameCanvas::FlameDraw(const int width, const int lenght,
                             GRADIENT_SIZE &size[],
                             GRADIENT_COLOR &gradient[]) {
...
void CFlameCanvas::FlameSet(datetime tb1, double pb1, datetime te1, double pe1,
                            datetime tb2, double pb2, datetime te2,
                            double pe2) {
...
void CFlameCanvas::FlameCreate(void) {
  ...
  w = (m_yb2 - m_yb1 != 0) ? w % (m_yb2 - m_yb1) : 10;
  if (w < 10)
    w = 10;
  l = l % ((m_xe1 - m_xb1) / (int)m_bar_gap - 20) + 20;
...
void CFlameCanvas::CloudDraw(const double &prices[], const int width,
                             const int lenght, GRADIENT_SIZE &size[],
                             GRADIENT_COLOR &gradient[], const uchar t_level,
                             const bool custom_gradient) {
...
void CFlameCanvas::GradientVertical(const int xb, const int xe, const int yb1,
                                    const int ye1, const int yb2, const int ye2,
                                    const GRADIENT_COLOR &gradient[]) {
...
void CFlameCanvas::GradientVerticalLineMonochrome(int x, int y1, int y2,
                                                  uint clr1, uint clr2) {
...
void CFlameCanvas::GradientVerticalLine(int x, int y1, int y2,
                                        const GRADIENT_COLOR &gradient[]) {
  ...
  for (int i = 0; i < total - 1; i++)
    GradientVerticalLineMonochrome(x, y1 + dy * gradient[i].pos / 100,
                                   y1 + dy * gradient[i + 1].pos / 100,
                                   gradient[i].clr, gradient[i + 1].clr);
}

void CFlameCanvas::ChartEventHandler(const int id, const long &lparam,
                                     const double &dparam,
                                     const string &sparam) {

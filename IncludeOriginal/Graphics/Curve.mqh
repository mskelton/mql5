...
CCurve::CCurve(const double &y[], const uint clr, ENUM_CURVE_TYPE type,
               const string name)
    : m_name(name), m_clr(clr), m_type(type), m_visible(false),
      m_lines_style(STYLE_SOLID), m_lines_end_style(LINE_END_ROUND),
      m_lines_width(1), m_lines_smooth(false), m_lines_tension(0.5),
      m_lines_step(1.0), m_points_size(6), m_points_type(POINT_CIRCLE),
      m_points_fill(false), m_points_clr(clr), m_steps_dimension(0),
      m_hisogram_width(1), m_custom_plot_func(NULL), m_custom_plot_cbdata(NULL),
      m_trend_visible(false), m_trend_clr(clr), m_trend_calc(false) {
...
CCurve::CCurve(const double &x[], const double &y[], const uint clr,
               ENUM_CURVE_TYPE type, const string name)
    : m_name(name), m_clr(clr), m_type(type), m_visible(false),
      m_lines_style(STYLE_SOLID), m_lines_end_style(LINE_END_ROUND),
      m_lines_width(1), m_lines_smooth(false), m_lines_tension(0.5),
...
CCurve::CCurve(const CPoint2D &points[], const uint clr, ENUM_CURVE_TYPE type,
               const string name)
    : m_name(name), m_clr(clr), m_type(type), m_visible(false),
      m_lines_style(STYLE_SOLID), m_lines_end_style(LINE_END_ROUND),
      m_lines_width(1), m_lines_smooth(false), m_lines_tension(0.5),
...
CCurve::CCurve(CurveFunction function, const double from, const double to,
               const double step, const uint clr, ENUM_CURVE_TYPE type,
               const string name)
    : m_name(name), m_clr(clr), m_type(type), m_visible(false),
      m_lines_style(STYLE_SOLID), m_lines_end_style(LINE_END_ROUND),
...
void CCurve::Update(CurveFunction function, const double from, const double to,
                    const double step) {
  m_trend_calc = false;
...
void CCurve::CalculateCoefficients(void) {
  ...
  m_trend_coeff[0] =
      (sum_xy - (m_size * xmean * ymean)) / (sum_xx - (m_size * xmean * xmean));

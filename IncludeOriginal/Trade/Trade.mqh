...
CTrade::CTrade(void)
    : m_async_mode(false), m_magic(0), m_deviation(10),
      m_type_filling(ORDER_FILLING_FOK), m_log_level(LOG_LEVEL_ERRORS) {
  SetMarginMode();
...
bool CTrade::PositionModify(const string symbol, const double sl,
                            const double tp) {
...
bool CTrade::PositionModify(const ulong ticket, const double sl,
                            const double tp) {
...
bool CTrade::PositionClosePartial(const string symbol, const double volume,
...
bool CTrade::PositionClosePartial(const ulong ticket, const double volume,
                                  const ulong deviation) {
...
bool CTrade::OrderOpen(const string symbol, const ENUM_ORDER_TYPE order_type,
                       const double volume, const double limit_price,
                       const double price, const double sl, const double tp,
  ...
  if (type_time == ORDER_TIME_GTC && expiration == 0) {
    ...
    if ((exp & SYMBOL_EXPIRATION_GTC) != SYMBOL_EXPIRATION_GTC) {
      ...
      if ((exp & SYMBOL_EXPIRATION_DAY) != SYMBOL_EXPIRATION_DAY) {
        ...
        Print(__FUNCTION__, ": Error: Unable to place order without explicitly "
...
bool CTrade::OrderModify(const ulong ticket, const double price,
                         const double sl, const double tp,
...
bool CTrade::Buy(const double volume, const string symbol = NULL,
  ...
  return (PositionOpen(symbol_name, ORDER_TYPE_BUY, volume, price, sl, tp,
                       comment));
}

bool CTrade::Sell(const double volume, const string symbol = NULL,
                  double price = 0.0, const double sl = 0.0,
                  const double tp = 0.0, const string comment = "") {
  ...
  return (PositionOpen(symbol_name, ORDER_TYPE_SELL, volume, price, sl, tp,
                       comment));
}

bool CTrade::BuyLimit(const double volume, const double price,
                      const string symbol = NULL, const double sl = 0.0,
                      const double tp = 0.0,
                      const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,
  ...
  return (OrderOpen(symbol_name, ORDER_TYPE_BUY_LIMIT, volume, 0.0, price, sl,
                    tp, type_time, expiration, comment));
}

bool CTrade::BuyStop(const double volume, const double price,
                     const string symbol = NULL, const double sl = 0.0,
                     const double tp = 0.0,
                     const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,
                     const datetime expiration = 0, const string comment = "") {
  ...
  return (OrderOpen(symbol_name, ORDER_TYPE_BUY_STOP, volume, 0.0, price, sl,
                    tp, type_time, expiration, comment));
}

bool CTrade::SellLimit(const double volume, const double price,
                       const string symbol = NULL, const double sl = 0.0,
                       const double tp = 0.0,
                       const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,
                       const datetime expiration = 0,
  ...
  return (OrderOpen(symbol_name, ORDER_TYPE_SELL_LIMIT, volume, 0.0, price, sl,
                    tp, type_time, expiration, comment));
}

bool CTrade::SellStop(const double volume, const double price,
                      const string symbol = NULL, const double sl = 0.0,
                      const double tp = 0.0,
                      const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,
                      const datetime expiration = 0,
  ...
  return (OrderOpen(symbol_name, ORDER_TYPE_SELL_STOP, volume, 0.0, price, sl,
                    tp, type_time, expiration, comment));
}
...
string CTrade::FormatOrderPrice(string &str, const double price_order,
...
string CTrade::FormatRequest(string &str,
                             const MqlTradeRequest &request) const {
  ...
  case TRADE_ACTION_DEAL:
    ...
    case SYMBOL_TRADE_EXECUTION_REQUEST:
      if (IsHedging() && request.position != 0)
        str =
            StringFormat("request %s %s position #%I64u %s at %s",
                         FormatOrderType(type, request.type),
                         DoubleToString(request.volume, 2), request.position,
                         request.symbol, DoubleToString(request.price, digits));
      else
      ...
      if (request.sl != 0.0) {
        tmp_string =
            StringFormat(" sl: %s", DoubleToString(request.sl, digits));
        str += tmp_string;
      }
      if (request.tp != 0.0) {
        tmp_string =
            StringFormat(" tp: %s", DoubleToString(request.tp, digits));
        str += tmp_string;
      }
      break;
    ...
    case SYMBOL_TRADE_EXECUTION_INSTANT:
      if (IsHedging() && request.position != 0)
        str =
            StringFormat("instant %s %s position #%I64u %s at %s",
                         FormatOrderType(type, request.type),
                         DoubleToString(request.volume, 2), request.position,
                         request.symbol, DoubleToString(request.price, digits));
      else
    ...
    case SYMBOL_TRADE_EXECUTION_MARKET:
      if (IsHedging() && request.position != 0)
        ...
        str =
            StringFormat("market %s %s %s", FormatOrderType(type, request.type),
                         DoubleToString(request.volume, 2), request.symbol);
  ...
  case TRADE_ACTION_PENDING:
    str = StringFormat(
        "%s %s %s at %s", FormatOrderType(type, request.type),
        DoubleToString(request.volume, 2), request.symbol,
  ...
  case TRADE_ACTION_SLTP:
    if (IsHedging() && request.position != 0)
      str = StringFormat("modify position #%I64u %s (sl: %s, tp: %s)",
                         request.position, request.symbol,
                         DoubleToString(request.sl, digits),
                         DoubleToString(request.tp, digits));
    else
      str = StringFormat("modify %s (sl: %s, tp: %s)", request.symbol,
                         DoubleToString(request.sl, digits),
                         DoubleToString(request.tp, digits));
    break;
  ...
  case TRADE_ACTION_MODIFY:
    str = StringFormat(
        "modify #%I64u at %s (sl: %s tp: %s)", request.order,
        FormatOrderPrice(price_new, request.price, request.stoplimit, digits),
        DoubleToString(request.sl, digits), DoubleToString(request.tp, digits));
    break;
  ...
  case TRADE_ACTION_CLOSE_BY:
    if (IsHedging() && request.position != 0)
      str = StringFormat("close position #%I64u by #%I64u", request.position,
                         request.position_by);
    else
      str = StringFormat("wrong action close by (#%I64u by #%I64u)",
...
string CTrade::FormatRequestResult(string &str, const MqlTradeRequest &request,
                                   const MqlTradeResult &result) const {
  ...
  case TRADE_RETCODE_REQUOTE:
    str = StringFormat("requote (%s/%s)", DoubleToString(result.bid, digits),
  ...
  case TRADE_RETCODE_DONE_PARTIAL:
    if (request.action == TRADE_ACTION_DEAL &&
      ...
      str = StringFormat("done partially %s", DoubleToString(result.volume, 2));
    break;
...
double CTrade::CheckVolume(const string symbol, double volume, double price,
                           ENUM_ORDER_TYPE order_type) {
  ...
  if (!::OrderCheck(m_request, m_check_result) &&
      m_check_result.margin_free < 0.0) {
...
bool CTrade::OrderCheck(const MqlTradeRequest &request,
                        MqlTradeCheckResult &check_result) {
...
bool CTrade::FillingCheck(const string symbol) {
  ...
  ENUM_SYMBOL_TRADE_EXECUTION exec =
      (ENUM_SYMBOL_TRADE_EXECUTION)SymbolInfoInteger(symbol,
                                                     SYMBOL_TRADE_EXEMODE);

  if (exec == SYMBOL_TRADE_EXECUTION_REQUEST ||
  ...
  case ORDER_FILLING_FOK:
    ...
    if (m_request.action == TRADE_ACTION_PENDING) {
      ...
      if (m_request.type == ORDER_TYPE_BUY_STOP ||
          m_request.type == ORDER_TYPE_SELL_STOP ||
          m_request.type == ORDER_TYPE_BUY_LIMIT ||
          m_request.type == ORDER_TYPE_SELL_LIMIT) {
...
bool CTrade::OrderSend(const MqlTradeRequest &request, MqlTradeResult &result) {
  ...
  if (res) {
    if (m_log_level > LOG_LEVEL_ERRORS)
      PrintFormat(__FUNCTION__ + ": %s [%s]", FormatRequest(action, request),
                  FormatRequestResult(fmt, request, result));
  } else {

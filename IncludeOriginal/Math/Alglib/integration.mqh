...
//| INPUT PARAMETERS:                                                |
//|     Alpha   â   array[0..N-1], alpha coefficients                |
//|     Beta    â   array[0..N-1], beta coefficients                 |
//|                 Zero-indexed element is not used and may be      |
//|                 arbitrary. Beta[I]>0.                            |
//|     Mu0     â   zeroth moment of the weight function.            |
//|     N       â   number of nodes of the quadrature formula, N>=1  |
//| OUTPUT PARAMETERS:                                               |
//|     Info    -   error code:                                      |
...
//| Mu0 = integral(W(x)dx,a,b)                                       |
//| INPUT PARAMETERS:                                                |
//|     Alpha   â   array[0..N-2], alpha coefficients                |
//|     Beta    â   array[0..N-2], beta coefficients.                |
//|                 Zero-indexed element is not used, may be         |
//|                 arbitrary. Beta[I]>0                             |
//|     Mu0     â   zeroth moment of the weighting function.         |
//|     A       â   left boundary of the integration interval.       |
//|     B       â   right boundary of the integration interval.      |
//|     N       â   number of nodes of the quadrature formula, N>=3  |
...
//|     Mu0     â   zeroth moment of the weighting function.         |
//|     A       â   left boundary of the integration interval.       |
//|     N       â   number of nodes of the quadrature formula, N>=2  |
...
//|     Mu0 = integral(W(x)dx,a,b)                                   |
//| INPUT PARAMETERS:                                                |
//|     Alpha       â   alpha coefficients, array[0..floor(3*K/2)].  |
//|     Beta        â   beta coefficients,  array[0..ceil(3*K/2)].   |
//|                     Beta[0] is not used and may be arbitrary.    |
//|                     Beta[I]>0.                                   |
//|     Mu0         â   zeroth moment of the weight function.        |
//|     N           â   number of nodes of the Gauss-Kronrod         |
//|                     quadrature formula,                          |
//|                     N >= 3,                                      |
//|                     N =  2*K+1.                                  |
//| OUTPUT PARAMETERS:                                               |
...
static void CGaussKronrodQ::GKQGenerateRec(double &calpha[], double &cbeta[],
  ...
  //--- Algorithm from Dirk P. Laurie,"Calculation of Gauss-Kronrod quadrature
  //rules",1997.
  t[woffs + 0] = beta[n + 1];
  for (m = 0; m <= n - 2; m++) {
    u = 0;
...
static bool CAutoGK::AutoGKInternalIteration(CAutoGKInternalState &state) {
  ...
  //--- * column 1   -   integral of a F(x) (calculated using Kronrod extension
  //nodes)
  //--- * column 2   -   integral of a |F(x)| (calculated using modified rect.
  //method)
...
static bool CAutoGK::AutoGKIteration(CAutoGKState &state) {
  ...
  if (state.m_wrappermode != 0) {
    ...
    //---     =1/(1+alpha) *
    //integral(t^(-alpha/(1+alpha))*f(a+t^(1/(1+alpha)))dt,0,(0.5*(b-a))^(1+alpha))
...
static bool CAutoGK::Func_lbl_9(CAutoGKState &state, double &s, double &tmp,
  ...
  if (!AutoGKInternalIteration(state.m_internalstate)) {
    ...
    //---     =1/(1+beta) *
    //integral(t^(-beta/(1+beta))*f(b-t^(1/(1+beta)))dt,0,(0.5*(b-a))^(1+beta))

...
static bool CBaseStat::SampleMoments(const double &cx[], const int n,
  ...
  if (!CAp::Assert(CApServ::IsFiniteVector(cx, n),
                   __FUNCTION__ + ": x is not finite vector"))
    return (false);
...
static double CBaseStat::Cov2(const double &cx[], const double &cy[],
  ...
  if (!CAp::Assert(CApServ::IsFiniteVector(cy, n),
                   __FUNCTION__ + ": y is not finite vector"))
    return (EMPTY_VALUE);
...
static bool CBaseStat::CovM(const CMatrixDouble &cx, const int n, const int m,
  ...
  if (!CAp::Assert(CApServ::IsFiniteMatrix(cx, n, m),
                   __FUNCTION__ + ": x contains infinite/NAN elements"))
    return (false);
  ...
  CAblas::RMatrixSyrk(m, n, 1.0 / (double)(n - 1), x, 0, 0, 1, 0.0, c, 0, 0,
                      true);
...
static bool CBaseStat::CovM2(const CMatrixDouble &cx, const CMatrixDouble &cy,
  ...
  if (!CAp::Assert(CAp::Cols(cx) >= m1 || n == 0,
                   __FUNCTION__ + ": cols(x)<m1"))
    return (false);

  if (!CAp::Assert(CApServ::IsFiniteMatrix(cx, n, m1),
                   __FUNCTION__ + ": x contains infinite/NAN elements"))
    return (false);
  ...
  if (!CAp::Assert(CAp::Cols(cy) >= m2 || n == 0,
                   __FUNCTION__ + ": cols(y)<m2"))
    return (false);

  if (!CAp::Assert(CApServ::IsFiniteMatrix(cy, n, m2),
                   __FUNCTION__ + ": y contains infinite/NAN elements"))
    return (false);
  ...
  CAblas::RMatrixGemm(m1, m2, n, 1.0 / (double)(n - 1), x, 0, 0, 1, y, 0, 0, 0,
                      0.0, c, 0, 0);

  return (true);
}

static bool CBaseStat::PearsonCorrM2(const CMatrixDouble &cx,
                                     const CMatrixDouble &cy, const int n,
...
static bool CBaseStat::SpearmanCorrM2(const CMatrixDouble &cx,
                                      const CMatrixDouble &cy, const int n,
                                      const int m1, const int m2,
                                      CMatrixDouble &c) {
...
static bool CJarqueBera::JarqueBeraStat(const double &x[], const int n,
  ...
  if (stddev != 0) {
    ...
    s = (double)n / (double)6 *
        (CMath::Sqr(skewness) + CMath::Sqr(kurtosis) / 4);
  } else
    return (false);
...
static double CJarqueBera::JarqueBeraApprox(const int n, const double s) {
  ...
  result = -(0.5 * x) +
           (JBTbl1401(x) + 0.5 * x) * MathSqrt((double)1401 / (double)n);
...
static void CSignTest::OneSampleSignTest(const double &x[], const int n,
  ...
  bothTails = 2 * CBinomialDistr::BinomialDistribution(
                      MathMin(greater, noteql - greater), noteql, 0.5);
  leftTail = CBinomialDistr::BinomialDistribution(greater, noteql, 0.5);
  rightTail =
      CBinomialDistr::BinomialComplDistribution(greater - 1, noteql, 0.5);
}
...
static void CStudentTests::UnequalVarianceTest(const double &x[], const int n,
  ...
  df = (n - 1) * (m - 1) /
       ((m - 1) * CMath::Sqr(c) + (n - 1) * CMath::Sqr(1 - c));

  if (stat > 0)
    p = 1 - 0.5 * CIncBetaF::IncompleteBeta(df / 2, 0.5,
                                            df / (df + CMath::Sqr(stat)));
  else
    p = 0.5 *
        CIncBetaF::IncompleteBeta(df / 2, 0.5, df / (df + CMath::Sqr(stat)));
...
static void CVarianceTests::FTest(const double &x[], const int n,
  ...
  bothTails = 1 - (CFDistr::FDistribution(df1, df2, 1 / stat) -
                   CFDistr::FDistribution(df1, df2, stat));

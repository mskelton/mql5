...
int AngleArc(HANDLE hdc, int x, int y, uint r, float StartAngle,
             float SweepAngle);
int AnimatePalette(HANDLE pal, uint start_index, uint entries,
                   PALETTEENTRY &ppe);
int Arc(HANDLE hdc, int x1, int y1, int x2, int y2, int x3, int y3, int x4,
        int y4);
int ArcTo(HANDLE hdc, int left, int top, int right, int bottom, int xr1,
          int yr1, int xr2, int yr2);
int BeginPath(HANDLE hdc);
int BitBlt(HANDLE hdc, int x, int y, int cx, int cy, HANDLE src, int x1, int y1,
           uint rop);
int CancelDC(HANDLE hdc);
int CheckColorsInGamut(HANDLE hdc, RGBTRIPLE &lpRGBTriple, PVOID buffer,
                       uint count);
int ChoosePixelFormat(HANDLE hdc, PIXELFORMATDESCRIPTOR &ppfd);
int Chord(HANDLE hdc, int x1, int y1, int x2, int y2, int x3, int y3, int x4,
          int y4);
...
HANDLE CreateBitmap(int width, int height, uint planes, uint bit_count,
                    PVOID bits);
HANDLE CreateBitmap(int width, int height, uint planes, uint bit_count,
                    char bits[]);
...
HANDLE CreateDCW(const string driver, const string device, const string port,
                 DEVMODEW &pdm);
HANDLE CreateDIBitmap(HANDLE hdc, BITMAPINFOHEADER &pbmih, uint init,
                      PVOID bits, BITMAPINFO &pbmi, uint usage);
HANDLE CreateDIBPatternBrush(HANDLE h, uint usage);
HANDLE CreateDIBPatternBrushPt(PVOID lpPackedDIB, uint usage);
HANDLE CreateDIBSection(HANDLE hdc, BITMAPINFO &pbmi, uint usage, PVOID bits,
                        HANDLE section, uint offset);
...
HANDLE CreateEnhMetaFileW(HANDLE hdc, const string filename, RECT &lprc,
                          const string desc);
HANDLE CreateFontIndirectExW(ENUMLOGFONTEXDVW &);
HANDLE CreateFontIndirectW(LOGFONTW &lplf);
HANDLE CreateFontW(int height, int width, int escapement, int orientation,
                   int weight, uint italic, uint underline, uint strike_out,
                   uint char_set, uint out_precision, uint clip_precision,
                   uint quality, uint pitch_and_family, const string face_name);
HANDLE CreateHalftonePalette(HANDLE hdc);
HANDLE CreateHatchBrush(int hatch, uint clr);
HANDLE CreateICW(const string driver, const string device, const string port,
                 DEVMODEW &pdm);
...
HANDLE CreatePolyPolygonRgn(const POINT pptl[], const int pc[], int poly,
                            int mode);
...
int CreateScalableFontResourceW(uint hidden, const string font,
                                const string file, const string path);
...
int EnumEnhMetaFile(HANDLE hdc, HANDLE hmf, PVOID proc, PVOID param,
                    RECT &rect);
int EnumFontFamiliesExW(HANDLE hdc, LOGFONTW &logfont, PVOID proc, PVOID param,
                        uint flags);
int EnumFontFamiliesW(HANDLE hdc, const string logfont, PVOID proc,
                      PVOID param);
...
int ExtEscape(HANDLE hdc, int escape, int inp, uchar in_data[], int output,
              uchar out_data[]);
int ExtFloodFill(HANDLE hdc, int x, int y, uint clr, uint type);
int ExtSelectClipRgn(HANDLE hdc, HANDLE hrgn, int mode);
int ExtTextOutW(HANDLE hdc, int x, int y, uint options, const RECT &lprect,
                const string str, uint c, const int dx[]);
...
int GetDIBits(HANDLE hdc, HANDLE hbm, uint start, uint lines, PVOID bits,
...
uint GetEnhMetaFileHeader(HANDLE hemf, uint size,
                          ENHMETAHEADER &enh_meta_header);
uint GetEnhMetaFilePaletteEntries(HANDLE hemf, uint num_entries,
...
uint GetPaletteEntries(HANDLE hpal, uint start, uint entries,
...
uint GetSystemPaletteEntries(HANDLE hdc, uint start, uint entries,
                             PALETTEENTRY &pal_entries);
...
int Pie(HANDLE hdc, int left, int top, int right, int bottom, int xr1, int yr1,
        int xr2, int yr2);
int PlayEnhMetaFile(HANDLE hdc, HANDLE hmf, RECT &lprect);
int PlayEnhMetaFileRecord(HANDLE hdc, HANDLETABLE &pht, ENHMETARECORD &pmr,
                          uint cht);
...
int RoundRect(HANDLE hdc, int left, int top, int right, int bottom, int width,
              int height);
...
uint SetPaletteEntries(HANDLE hpal, uint start, uint entries,
                       PALETTEENTRY &pal_entries);
...
HANDLE SetWinMetaFileBits(uint size, const uchar lpMeta16Data[], HANDLE ref,

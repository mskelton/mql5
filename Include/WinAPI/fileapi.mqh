...
enum STREAM_INFO_LEVELS { FindStreamInfoStandard, FindStreamInfoMaxInfoLevel };
...
HANDLE CreateFile2(const string file_name, uint desired_access, uint share_mode,
                   ...
                   CREATEFILE2_EXTENDED_PARAMETERS &create_ex_params);
...
int DefineDosDeviceW(uint flags, const string device_name,
...
HANDLE FindFirstChangeNotificationW(const string path_name, int watch_subtree,
                                    uint notify_filter);
HANDLE FindFirstFileExW(const string file_name,
...
int FindNextFileNameW(HANDLE find_stream, uint &StringLength,
                      ushort LinkName[]);
...
int FindNextVolumeW(HANDLE find_volume, ushort volume_name[],
                    uint &buffer_length);
...
int GetDiskFreeSpaceExW(const string directory_name,
                        ulong &free_bytes_available_to_caller,
                        ulong &total_number_of_bytes,
                        ulong &total_number_of_free_bytes);
...
uint GetTempFileNameW(const string path_name, const string prefix_string,
                      uint unique, ushort temp_file_name[]);
uint GetTempPathW(uint buffer_length, ushort buffer[]);
int GetVolumeInformationByHandleW(HANDLE file, ushort volume_name_buffer[],
...
int GetVolumeInformationW(const string root_path_name,
                          ushort volume_name_buffer[], uint volume_name_size,
                          uint &volume_serial_number,
                          uint &maximum_component_length,
                          uint &file_system_flags,
                          ushort file_system_name_buffer[],
                          uint file_system_name_size);
int GetVolumeNameForVolumeMountPointW(const string volume_mount_point,
                                      string volume_name, uint buffer_length);
int GetVolumePathNamesForVolumeNameW(const string volume_name,
                                     string volume_path_names,
                                     uint buffer_length, uint &return_length);
int GetVolumePathNameW(const string file_name, ushort volume_path_name[],
                       uint buffer_length);
...
int LockFile(HANDLE file, uint file_offset_low, uint file_offset_high,
             uint number_of_bytes_to_lock_low,
             uint number_of_bytes_to_lock_high);
int LockFileEx(HANDLE file, uint flags, uint reserved,
               uint number_of_bytes_to_lock_low,
...
int ReadFileScatter(HANDLE file, FILE_SEGMENT_ELEMENT segment_array[],
                    uint number_of_bytes_to_read, uint &reserved,
...
int SetFileInformationByHandle(HANDLE file,
                               FILE_INFO_BY_HANDLE_CLASS FileInformationClass,
                               FILE_INFO &file_information, uint buffer_size);
int SetFileIoOverlappedRange(HANDLE FileHandle, uchar &OverlappedRangeStart,
                             uint Length);
uint SetFilePointer(HANDLE file, long distance_to_move,
                    long &distance_to_move_high, uint move_method);
int SetFilePointerEx(HANDLE file, long distance_to_move, long &new_file_pointer,
                     uint move_method);
int SetFileTime(HANDLE file, FILETIME &creation_time,
                FILETIME &last_access_time, FILETIME &last_write_time);
...
int UnlockFile(HANDLE file, uint file_offset_low, uint file_offset_high,
               uint number_of_bytes_to_unlock_low,

...
enum HANDEDNESS { HANDEDNESS_LEFT = 0, HANDEDNESS_RIGHT };
...
int AdjustWindowRectExForDpi(RECT &rect, uint style, int menu, uint ex_style,
                             uint dpi);
int AllowSetForegroundWindow(uint process_id);
int AnimateWindow(HANDLE wnd, uint time, uint flags);
int AnyPopup(void);
int AppendMenuW(HANDLE menu, uint flags, ulong uIDNewItem,
...
int ChangeMenuW(HANDLE menu, uint cmd, const string new_item, uint insert,
                uint flags);
int ChangeWindowMessageFilter(uint message, uint flag);
int ChangeWindowMessageFilterEx(HANDLE hwnd, uint message, uint action,
...
int CheckMenuRadioItem(HANDLE hmenu, uint first, uint last, uint check,
                       uint flags);
int CheckRadioButton(HANDLE dlg, int nIDFirstButton, int nIDLastButton,
                     int nIDCheckButton);
...
int CopyAcceleratorTableW(HANDLE accel_src, ACCEL &accel_dst,
                          int accel_entries);
...
HANDLE CreateDesktopExW(const string desktop, const PVOID device, PVOID devmode,
                        ...
                        uint heap_size, PVOID pvoid);
HANDLE CreateDesktopExW(const string desktop, const string device,
                        PVOID devmode, uint flags, uint desired_access,
                        PVOID lpsa, uint heap_size, PVOID pvoid);
HANDLE CreateDesktopExW(const string desktop, const PVOID device,
                        DEVMODEW &devmode, uint flags, uint desired_access,
                        PVOID lpsa, uint heap_size, PVOID pvoid);
HANDLE CreateDesktopExW(const string desktop, const string device,
                        DEVMODEW &devmode, uint flags, uint desired_access,
                        PVOID lpsa, uint heap_size, PVOID pvoid);
HANDLE CreateDesktopW(const string desktop, const PVOID device, PVOID devmode,
                      uint flags, uint desired_access, PVOID lpsa);
HANDLE CreateDesktopW(const string desktop, const string device, PVOID devmode,
                      uint flags, uint desired_access, PVOID lpsa);
HANDLE CreateDesktopW(const string desktop, const PVOID device,
                      DEVMODEW &devmode, uint flags, uint desired_access,
                      PVOID lpsa);
HANDLE CreateDesktopW(const string desktop, const string device,
                      DEVMODEW &devmode, uint flags, uint desired_access,
                      PVOID lpsa);
HANDLE CreateDialogIndirectParamW(HANDLE instance,
                                  const DLGTEMPLATE &dlg_template,
                                  HANDLE wnd_parent, PVOID dialog_func,
                                  PVOID init_param);
HANDLE CreateDialogParamW(HANDLE instance, const string template_name,
                          HANDLE wnd_parent, PVOID dialog_func,
                          PVOID init_param);
HANDLE CreateIcon(HANDLE instance, int width, int height, uchar planes,
                  uchar bits_pixel, const uchar lpbANDbits[],
                  const uchar lpbXORbits[]);
...
HANDLE CreateMDIWindowW(const string class_name, const string window_name,
                        ...
                        HANDLE wnd_parent, HANDLE instance, PVOID param);
HANDLE CreateMenu(void);
HANDLE CreatePopupMenu(void);
HANDLE CreateWindowExW(uint ex_style, const PVOID class_name,
                       const PVOID window_name, uint style, int X, int Y,
                       int width, int height, HANDLE wnd_parent, HANDLE menu,
                       HANDLE instance, PVOID param);
HANDLE CreateWindowExW(uint ex_style, const string class_name,
                       const string window_name, uint style, int X, int Y,
                       int width, int height, HANDLE wnd_parent, HANDLE menu,
                       HANDLE instance, PVOID param);
...
long DialogBoxIndirectParamW(HANDLE instance, DLGTEMPLATE &dialog_template,
                             HANDLE wnd_parent, PVOID dialog_func,
                             PVOID init_param);
long DialogBoxParamW(HANDLE instance, const string template_name,
                     HANDLE wnd_parent, PVOID dialog_func, PVOID init_param);
...
int DlgDirListComboBoxW(HANDLE dlg, string path_spec, int nIDComboBox,
                        int nIDStaticPath, uint filetype);
int DlgDirListW(HANDLE dlg, string path_spec, int nIDListBox, int nIDStaticPath,
                uint file_type);
...
int EnumDisplayDevicesW(const string device, uint dev_num,
                        DISPLAY_DEVICEW &display_device, uint flags);
int EnumDisplayMonitors(HANDLE hdc, const RECT &clip, PVOID enum_obj,
                        PVOID data);
int EnumDisplaySettingsExW(const string device_name, uint mode_num,
                           DEVMODEW &dev_mode, uint flags);
int EnumDisplaySettingsW(const string device_name, uint mode_num,
                         DEVMODEW &dev_mode);
...
int EvaluateProximityToPolygon(
    uint vertices, const POINT polygon[],
...
HANDLE FindWindowExW(HANDLE wnd_parent, HANDLE wnd_child_after,
                     const string class_name, const string window);
...
int GetClassInfoExW(HANDLE instance, const string class_name,
                    WNDCLASSEXW &lpwcx);
int GetClassInfoW(HANDLE instance, const string class_name,
                  WNDCLASSW &wnd_class);
...
GetDialogControlDpiChangeBehavior(HANDLE wnd);
...
int GetDisplayConfigBufferSizes(uint flags, uint &path_array_elements,
                                uint &mode_info_array_elements);
...
int GetLayeredWindowAttributes(HANDLE hwnd, uint &key, uchar &alpha,
                               uint &flags);
...
int GetMouseMovePointsEx(uint size, MOUSEMOVEPOINT &lppt, MOUSEMOVEPOINT &buf,
                         int buf_points, uint resolution);
...
uint GetRawInputData(HANDLE raw_input, uint command, PVOID data, uint &size,
                     uint size_header);
uint GetRawInputDeviceInfoW(HANDLE device, uint command, PVOID data,
                            uint &size);
...
uint GetTabbedTextExtentW(HANDLE hdc, const string str, int count,
...
int GetUserObjectInformationW(HANDLE obj, int index, PVOID info, uint length,
                              uint &length_needed);
int GetUserObjectSecurity(HANDLE obj, uint &pSIRequested,
                          SECURITY_DESCRIPTOR &pSID, uint length,
                          uint &length_needed);
...
int GetWindowFeedbackSetting(HANDLE hwnd, FEEDBACK_TYPE feedback, uint flags,
                             uint size, int &config);
...
uint GetWindowModuleFileNameW(HANDLE hwnd, ushort file_name[],
                              uint file_name_max);
...
int GrayStringW(HANDLE hDC, HANDLE brush, PVOID output_func, uchar data[],
                int count, int X, int Y, int width, int height);
int GrayStringW(HANDLE hDC, HANDLE brush, PVOID output_func, PVOID data,
                int count, int X, int Y, int width, int height);
...
int InsertMenuItemW(HANDLE hmenu, uint item, int by_position,
                    const MENUITEMINFO &lpmi);
int InsertMenuW(HANDLE menu, uint position, uint flags, ulong uIDNewItem,
                const string new_item);
...
HANDLE LoadImageW(HANDLE inst, const string name, uint type, int cx, int cy,
                  uint load);
...
int MapWindowPoints(HANDLE wnd_from, HANDLE wnd_to, POINT points[],
                    uint points_count);
int MenuItemFromPoint(HANDLE wnd, HANDLE menu, POINT &screen);
int MessageBeep(uint type);
int MessageBoxExW(HANDLE wnd, const string text, const string caption,
                  uint type, ushort language_id);
int MessageBoxIndirectW(MSGBOXPARAMSW &lpmbp);
int MessageBoxW(HANDLE wnd, const string text, const string caption, uint type);
int ModifyMenuW(HANDLE mnu, uint position, uint flags, ulong uIDNewItem,
                const string new_item);
...
HANDLE OpenDesktopW(const string desktop, uint flags, int inherit,
                    uint desired_access);
int OpenIcon(HANDLE wnd);
HANDLE OpenInputDesktop(uint flags, int inherit, uint desired_access);
HANDLE OpenWindowStationW(const string win_sta, int inherit,
                          uint desired_access);
PVOID PackTouchHitTestingProximityEvaluation(
    const TOUCH_HIT_TESTING_INPUT hit_testing_input[],
    const TOUCH_HIT_TESTING_PROXIMITY_EVALUATION proximity_eval[]);
int PaintDesktop(HANDLE hdc);
int PeekMessageW(MSG &msg, HANDLE wnd, uint msg_filter_min, uint msg_filter_max,
                 uint remove_msg);
...
PVOID RegisterDeviceNotificationW(HANDLE recipient, PVOID NotificationFilter,
                                  uint Flags);
...
PVOID RegisterPowerSettingNotification(HANDLE recipient,
                                       const GUID &PowerSettingGuid,
                                       uint Flags);
int RegisterRawInputDevices(const RAWINPUTDEVICE raw_input_devices[],
                            uint num_devices, uint size);
...
ulong SetCoalescableTimer(HANDLE wnd, ulong nIDEvent, uint elapse,
                          PVOID timer_func, uint tolerance_delay);
HANDLE SetCursor(HANDLE cursor);
int SetCursorPos(int X, int Y);
void SetDebugErrorLevel(uint level);
int SetDialogControlDpiChangeBehavior(
    HANDLE wnd, DIALOG_CONTROL_DPI_CHANGE_BEHAVIORS mask,
    DIALOG_CONTROL_DPI_CHANGE_BEHAVIORS values);
int SetDialogDpiChangeBehavior(HANDLE dlg, DIALOG_DPI_CHANGE_BEHAVIORS mask,
                               DIALOG_DPI_CHANGE_BEHAVIORS values);
int SetDisplayAutoRotationPreferences(ORIENTATION_PREFERENCE orientation);
int SetDisplayConfig(uint path_array_elements, DISPLAYCONFIG_PATH_INFO &array,
                     uint mode_info_array_elements,
                     DISPLAYCONFIG_MODE_INFO info_array[], uint flags);
...
int SetGestureConfig(HANDLE hwnd, uint reserved, uint cIDs,
                     GESTURECONFIG gesture_config[], uint size);
...
int SetMenuItemBitmaps(HANDLE menu, uint position, uint flags,
                       HANDLE bitmap_unchecked, HANDLE bitmap_checked);
int SetMenuItemInfoW(HANDLE hmenu, uint item, int by_positon,
...
int SetUserObjectSecurity(HANDLE obj, uint pSIRequested,
                          SECURITY_DESCRIPTOR &pSID);
int SetWindowContextHelpId(HANDLE, uint);
int SetWindowDisplayAffinity(HANDLE wnd, uint affinity);
int SetWindowFeedbackSetting(HANDLE hwnd, FEEDBACK_TYPE feedback, uint flags,
                             uint size, PVOID configuration);
...
int SetWindowPos(HANDLE wnd, HANDLE wnd_insert_after, int X, int Y, int cx,
                 int cy, uint flags);
...
int ToAsciiEx(uint virt_key, uint scan_code, const uchar key_state[],
              ushort &symbol, uint flags, HANDLE dwhkl);
int ToUnicode(uint virt_key, uint scan_code, const uchar key_state[],
              ushort buff[], int buff_size, uint flags);
int ToUnicodeEx(uint virt_key, uint scan_code, const uchar key_state[],
                ushort buff[], int buff_size, uint flags, HANDLE dwhkl);
int TrackMouseEvent(TRACKMOUSEEVENT &event_track);
int TrackPopupMenu(HANDLE menu, uint flags, int x, int y, int reserved,
                   HANDLE wnd, RECT &rect);
int TrackPopupMenuEx(HANDLE menu, uint flags, int x, int y, HANDLE hwnd,
                     TPMPARAMS &lptpm);

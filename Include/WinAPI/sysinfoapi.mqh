...
int GetSystemTimeAdjustment(uint &time_adjustment, uint &time_increment,
                            int &time_adjustment_disabled);
...
int GetComputerNameExW(COMPUTER_NAME_FORMAT name_type, ushort buffer[],
                       uint &size);
...
int GetLogicalProcessorInformation(
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION buffer[], uint &returned_length);
int GetLogicalProcessorInformationEx(
    LOGICAL_PROCESSOR_RELATIONSHIP relationship_type,
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX buffer[], uint &returned_length);
void GetNativeSystemInfo(SYSTEM_INFO &system_info);
void GetSystemTimePreciseAsFileTime(FILETIME &system_time_as_file_time);
int GetProductInfo(uint os_major_version, uint os_minor_version,
                   uint sp_major_version, uint sp_minor_version,
                   uint &returned_product_type);
uint EnumSystemFirmwareTables(uint firmware_table_provider_signature,
                              PVOID &firmware_table_enum_buffer,
                              uint buffer_size);
uint EnumSystemFirmwareTables(uint firmware_table_provider_signature,
                              uchar firmware_table_enum_buffer[],
                              uint buffer_size);
uint GetSystemFirmwareTable(uint firmware_table_provider_signature,
                            uint firmware_table_id, PVOID firmware_table_buffer,
                            uint buffer_size);
uint GetSystemFirmwareTable(uint firmware_table_provider_signature,
                            uint firmware_table_id,
                            uchar firmware_table_buffer[], uint buffer_size);
int DnsHostnameToComputerNameExW(const string hostname, ushort computer_name[],
                                 uint &size);
int GetPhysicallyInstalledSystemMemory(ulong &total_memory_in_kilobytes);
int SetComputerNameEx2W(COMPUTER_NAME_FORMAT name_type, uint flags,
                        const string buffer);
...
int GetProcessorSystemCycleTime(ushort group, PVOID &buffer,
